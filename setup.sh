...
check_prereq() {
    ...
    if [ -z "$HOST_IP" ] || [ -z "$IDP" ] || [ -z "$IDP_ID" ] || [ -z "$IDP_SECRET" ]; then
        ...
        check_and_prompt "HOST_IP" "Enter the host IP: "
...
controller:
  ...
  extraArgs:
    default-ssl-certificate: "watkins/$URL-tls"
  hostNetwork: true
  hostPort:
    enabled: true
  service:
...
install_app() {
    ...
    URL="$(convert_ip_to_dash "$HOST_IP").ip.daytona.io"
    ...
    check_helm_release watkins watkins
    kubectl get secret "${URL}"-tls -n watkins -o jsonpath="{.data.tls\.crt}" | base64 --decode >wildcard."${URL}".crt
    echo -e "\n--------------------------------------------------------------------------------------------------\n"
    echo -e "${INFO} A self-signed wildcard certificate has been generated. To trust this certificate locally, you need to perform one of the following manual tasks:"
    echo -e "  1. Download and import the certificate directly from your machine. The certificate is stored with the filename wildcard.${URL}.crt."
    echo -e "  2. Visit the Daytona dashboard, navigate to the appropriate section, and manually download and import the certificate using your browser."
    echo -e "\n--------------------------------------------------------------------------------------------------\n"
